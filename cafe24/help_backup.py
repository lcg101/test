...
def get_new_server_prefix(original_prefix):
    if not re.match(r'^q\d{3}-\d+$', original_prefix):
        return original_prefix  # If it's not in the format 'qxxx-xxxx', just return the original prefix
...
def run_server_recovery(input_value, nat_server_info, password_info, counter):
    short_hostnames = Server_recovery.main(input_value)  # Server_recovery의 main 함수 호출
    if not short_hostnames:
        print(f"Server_recovery.py 실행 중 오류 발생: {input_value}")
        return None

    short_hostname = short_hostnames[counter % len(short_hostnames)]  # 두 번째 사용자 ID부터 +1 증가
    result_lines = []
    result_lines.append(f"/ {short_hostname} / OS: 현황확인필요 / 추가예정")
...
def process_user_id(user_id, result_lines, counter):
    ...
    # 사용 가능한 서버 번호 할당 및 증가
    server_info = run_server_recovery(input_value, nat_server_info, password_info, counter)
    if server_info is None:
        print(f"Server recovery.py 실행 중 오류가 발생했습니다: {user_id}")
        return

    result_lines.append(f"\n최종 출력 결과 for {user_id}:")
    result_lines.append(f"{user_id} : {user_status} / {input_value} / {server_info}")
...
def main():
    ...
    for user_id in user_id_list:
        process_user_id(user_id, result_lines, counter)
        counter += 1
